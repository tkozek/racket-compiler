...
(define-syntax-rule (check-by-interp p)
  (check-equal? (interp-proc-imp-cmf-lang-v6 (check-output (normalize-bind p)))
                (interp-imp-mf-lang-v6 (check-input p))))
...
(check-by-interp '(module (define L.func.0.1
                          ...
                          (begin
                            ...
                            (if (not (not (< foo.5.5 foo.5.5)))
                                (begin
                                  (set! foobar.4.6 (if (< foo.5.5 0) 9223372036854775807 foo.5.5))
                                  -1170116362)
                                (* 0 0)))
                    ))
(check-by-interp

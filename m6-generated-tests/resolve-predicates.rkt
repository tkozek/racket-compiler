...
(define (check-input p)
  (if (block-pred-lang-v6? p)
      p
      (error
       (~a (pretty-format p) "\n is not a semantically valid " "block-pred-lang-v6" " program"))))
...
(define-syntax-rule (check-by-interp p)
  (check-equal? (interp-block-asm-lang-v6 (check-output (resolve-predicates p)))
                (interp-block-pred-lang-v6 (check-input p))))

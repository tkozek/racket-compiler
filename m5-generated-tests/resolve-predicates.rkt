...
(define (check-input p)
  (if (block-pred-lang-v4? p)
      p
      (error
       (~a (pretty-format p) "\n is not a semantically valid " "block-pred-lang-v4" " program"))))

(define (check-output p)
  (if (block-asm-lang-v4? p)
      p
      (error
       (~a (pretty-format p) "\n is not a semantically valid " "block-asm-lang-v4" " program"))))

(define-syntax-rule (check-by-interp p)
  (check-equal? (interp-block-pred-lang-v4 (check-input p))
                (interp-block-asm-lang-v4 (check-output (resolve-predicates p)))))
...
(check-resolve-predicates `(module (define L.start.1 (halt 5))) `(module (define L.start.1 (halt 5))))

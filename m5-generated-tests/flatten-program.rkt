...
(define (check-output p)
  (if (para-asm-lang-v4? p)
      p
      (error
       (~a (pretty-format p) "\n is not a semantically valid " "para-asm-lang-v4" " program"))))

(define-syntax-rule (check-flatten-program bal4 pal4)
  (check-equal? (flatten-program bal4) pal4))
(define-syntax-rule (check-by-interp p)
  (check-equal? (interp-block-asm-lang-v4 (check-input p))
                (interp-para-asm-lang-v4 (check-output (flatten-program p)))))

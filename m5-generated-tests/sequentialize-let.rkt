...
(define (check-input p)
  (if (values-unique-lang-v5? p)
      p
      (error
       (~a (pretty-format p) "\n is not a semantically valid " "values-unique-lang-v5" " program"))))
...
(define-syntax-rule (check-by-interp p)
  (check-equal? (interp-values-unique-lang-v5 (check-input p))
                (interp-imp-mf-lang-v5 (check-output (sequentialize-let p)))))
                            ...
                            (lambda (bat.1.6 foobar.5.5 foobar.8.4 foo.0.3 foo.3.2 foo.4.1)
                              (if (true)
                                  ...
                                  (if (true)
                                      ...
                                      (let ([foo.4.8 foobar.5.5]
                                            [foo.3.7 0])
                                        foo.0.3)))))
                          (if (let ([foo.4.10 -9223372036854775808]
                                    [ball.7.9 0])
                                (> foo.4.10 ball.7.9))
                              -9223372036854775808
                              (let ([bat.1.13 -9223372036854775808]
                                    [foo.4.12 1]
                                    [foobar.8.11 1])
                                foobar.8.11))
                    ))
(check-by-interp
 '(module (define L.tmp.0.1
            (lambda (bat.2.6 bat.0.5 foobar.5.4 foobar.6.3 bat.8.2 foo.9.1)
              (let ([foobar.6.8 (if (true)
                                    (* bat.0.5 -1194246923)
                                    foobar.5.4)]
                    [foobar.5.7 (if (let ([bat.2.10 foobar.5.4]
                                          [bat.4.9 foo.9.1])
                                      (= bat.2.10 422731415))
          ...
          (let ([bat.0.12 (+ -9223372036854775808 937267391)]
                [bat.2.11 (+ -9223372036854775808 -9223372036854775808)])
...
(check-by-interp '(module (define L.proc.0.1
                            (lambda (ball.1.3 bat.9.2 foobar.2.1)
                              (if (false)
                                  ...
                                  (let ([bat.7.6 (let ([bar.5.8 1554173211]
                                                       [foobar.2.7 616342412])
                                                   foobar.2.7)]
                                        [ball.1.5 (if (= foobar.2.1 ball.1.3) foobar.2.1 bat.9.2)]
                                        [ball.3.4 (if (= 1 0) bat.9.2 0)])
                    ...
                    (let ([foo.6.16 (let ([foobar.0.19 2142022224]
                                          [foo.6.18 0]
                                          [bat.7.17 1])
                                      foo.6.18)]
                          [foobar.2.15 (+ 1 121573080)]
                          [bat.9.14 (* 9223372036854775807 -1355489058)])
                      0)))
(check-by-interp
 '(module (let ([ball.5.3 (+ 0 9223372036854775807)]
                [ball.7.2 -678062995]
                [ball.8.1 (if (!= 0 9223372036854775807) -2004574473 9223372036854775807)])
            (* 1 ball.5.3))))
(check-by-interp
 '(module (define L.fn.0.1 (lambda (foo.3.3 foo.1.2 bat.4.1) (call L.fn.0.1 foo.1.2 bat.4.1 foo.1.2)))
          (define L.tmp.1.2
            (lambda ()
              (if (true)
                  (let ([ball.0.6 (let ([foo.1.8 -918704320]
                                        [foo.6.7 -1822595193])
                                    foo.6.7)]
                        [bat.4.5 0]
                        [foo.7.4 (* 9223372036854775807 810114007)])
                    (let ([foo.9.11 1]
                          [foo.7.10 foo.7.4]
                          [bat.8.9 1])
                      1271317139))
                  (if (let ([ball.0.13 9223372036854775807]
                            [foobar.5.12 -9223372036854775808])
                        (< 1474008962 ball.0.13))
                      (if (!= -566856990 -9223372036854775808) 1 1056302987)
                      (call L.tmp.1.2)))))
    (let ([foo.1.15 (let ([bat.4.17 -9223372036854775808]
                          [foo.7.16 9223372036854775807])
                      bat.4.17)]
          [foo.6.14 (if (>= -1846550218 1836975164) -186139937 1)])
      (let ([foo.7.19 1884722986]
            [foo.6.18 9223372036854775807])
        0))))
(check-by-interp '(module (define L.func.0.1
                          ...
                          (define L.func.1.2
                            (lambda (foobar.1.8 bat.0.7)
                              (let ([foobar.1.11 (let ([bat.8.13 0]
                                                       [bar.3.12 (let ([foo.2.16 foobar.1.8]
                                                                       [bat.8.15 1492712685]
                                                                       [foo.4.14 foobar.1.8])
                                                                   9223372036854775807)])
                                                   bar.3.12)]
                                    [bar.3.10 (let ([foobar.5.18 (let ([bat.8.21 bat.0.7]
                                                                       [foo.2.20 foobar.1.8]
                                                                       [foo.4.19 -383652955])
                                                                   -9223372036854775808)]
                                                    [foobar.1.17 (+ bat.0.7 566306668)])
...
(check-by-interp
 '(module (define L.fn.0.1
            (lambda ()
              (let ([bar.8.2 (* -9223372036854775808 0)]
                    [foobar.1.1 (let ([ball.3.5 (let ([foobar.7.7 0]
                                                      [bar.2.6 9223372036854775807])
                                                  foobar.7.7)]
                                      [foobar.0.4 (let ([bar.8.10 1]
                                                        [ball.3.9 -9223372036854775808]
                                                        [bat.4.8 0])
                                                    0)]
                                      [bar.8.3 (+ 2146083242 1877941467)])
                                  (if (>= 1 0) foobar.0.4 bar.8.3))])
                (let ([foobar.0.13 (let ([ball.6.16 9223372036854775807]
                                         [ball.3.15 1]
                                         [bat.4.14 bar.8.2])
                                     ball.3.15)]
                      [bar.2.12 (let ([foobar.1.19 foobar.1.1]
                                      [bar.8.18 -80057626]
                                      [bar.5.17 451403960])
                                  0)]
                      [ball.6.11 -9223372036854775808])
    ...
    (define L.func.2.3
      (lambda (foobar.0.30 bat.4.29 ball.3.28 bar.8.27)
        (let ([bar.5.33
               (let ([bar.2.36 (let ([bat.4.38 1683456577]
                                     [ball.3.37 0])
                                 572391381)]
                     [bar.8.35 (if (>= 9223372036854775807 foobar.0.30) -2023903597 -15104029)]
                     [foobar.7.34 1])
                 ball.3.28)]
              [ball.3.32 0]
              [bar.8.31 (+ bat.4.29 bat.4.29)])
          (+ 1 bat.4.29))))
    (if (not (> 512244517 -294049298))
        (let ([bar.5.41 1283186732]
              [bar.2.40 99039009]
              [bat.4.39 1811100583])
          bar.5.41)
        (let ([bat.4.44 1]
              [foobar.7.43 -9223372036854775808]
              [ball.6.42 1])
          ball.6.42))))
(check-by-interp
 '(module (define L.proc.0.1
          ...
          (define L.x.1.2
            (lambda (foobar.4.7 bar.2.6)
              (if (true)
                  (let ([foobar.0.10 (if (< foobar.4.7 foobar.4.7) foobar.4.7 foobar.4.7)]
                        [foo.5.9 (* foobar.4.7 -1298566605)]
                        [bar.2.8 0])
                    (* foobar.0.10 foobar.0.10))
                  (let ([foobar.4.12 (+ -1679672366 bar.2.6)]
                        [foobar.8.11 (+ 0 foobar.4.7)])
                    (let ([foobar.8.15 0]
                          [foobar.3.14 -511883263]
                          [bat.1.13 bar.2.6])
                      foobar.3.14)))))
    (define L.proc.2.3
...
(check-by-interp '(module (define L.x.0.1
                            (lambda (foobar.3.1)
                              (if (not (false))
                                  (let ([bat.7.4 (let ([bat.7.7 -508084756]
                                                       [foobar.3.6 foobar.3.1]
                                                       [bar.5.5 1])
                                                   bat.7.7)]
                                        [bat.4.3 (let ([bat.7.10 foobar.3.1]
                                                       [bar.5.9 1120136428]
                                                       [foobar.1.8 606194936])
                                                   -908142049)]
                                        [ball.0.2 (+ 600266106 foobar.3.1)])
                                    (let ([foobar.3.12 foobar.3.1]
                                          [ball.0.11 0])
                                      1))
                                  (call L.x.0.1 -2072909432))))
                          (let ([foobar.3.15 (let ([bat.7.18 -1755065230]
                                                   [bar.5.17 9223372036854775807]
                                                   [ball.0.16 0])
                                               bar.5.17)]
                                [bar.8.14 -9223372036854775808]
                                [bar.5.13 -9223372036854775808])
                            (if (< 1 bar.8.14) bar.8.14 -9223372036854775808))
                    ))
(check-by-interp '(module (let ([bar.4.3 (* 993422572 1)]
                                [bar.6.2 (if (<= 92310708 1290581674) 1 0)]
                                [bar.1.1 (+ 9223372036854775807 -1759699484)])
                            (let ([bar.4.5 bar.4.3]
                                  [foobar.5.4 -9223372036854775808])
                              bar.4.5))))
(check-by-interp '(module (define L.func.0.1
                            (lambda (bar.6.1)
                              (let ([bat.2.4 (let ([foo.0.6 (+ -9223372036854775808
                                                               -9223372036854775808)]
                                                   [bar.6.5 (+ bar.6.1 1312056768)])
 ...
 '(module (define L.proc.0.1
          ...
          (define L.func.1.2
            (lambda (bar.5.13 bat.6.12 bar.9.11 bar.4.10 foobar.7.9 ball.0.8 foobar.2.7)
              (let ([foobar.3.16 (* -9223372036854775808 ball.0.8)]
                    [foobar.2.15 ball.0.8]
                    [bat.6.14 bar.4.10])
                (call L.func.1.2
                      -9223372036854775808
                      0
                          ...
                          (define L.tmp.1.2
                            (lambda (bat.6.3)
                              (if (!= 9223372036854775807 bat.6.3)
                                  (if (true)
                                      (let ([bar.2.6 2026709313]
                                            [bat.9.5 bat.6.3]
                                            [bat.6.4 bat.6.3])
                                        bat.6.4)
                                      (if (>= 1890937388 bat.6.3) bat.6.3 1616264974))
                                  9223372036854775807)))
                    (let ([ball.7.9 (* 0 1192307430)]
                          [bat.3.8 (if (< -9223372036854775808 1) -9223372036854775808 2088471563)]
                          [bar.8.7 (let ([ball.7.11 1]
                                         [bat.9.10 9223372036854775807])
                                     1861500702)])
 ...
 '(module (define L.proc.0.1
            (lambda (foo.9.1)
              (if (false)
                  ...
                  (if (false)
                      (let ([foobar.8.4 9223372036854775807]
                            [foo.2.3 foo.9.1]
                            [foobar.1.2 foo.9.1])
                        foo.9.1)
          ...
          (define L.fn.1.2
            (lambda (foobar.1.7 foo.6.6 foobar.3.5)
              (if (false)
                  (let ([foobar.1.10 foobar.1.7]
                        [foobar.5.9 (+ -2065189484 foobar.3.5)]
                        [foobar.7.8 (if (< foobar.1.7 -300039187) 2115350249 foobar.1.7)])
                    (let ([foobar.1.13 -195312042]
                          [foobar.5.12 foo.6.6]
                          [foo.6.11 foobar.3.5])
                      foobar.7.8))
                  (let ([foo.6.15 (let ([foobar.3.17 -1983872386]
                                        [foobar.7.16 foobar.1.7])
                                    foobar.7.16)]
                        [foobar.3.14 (let ([foo.6.20 9223372036854775807]
                                           [foo.0.19 -9223372036854775808]
                                           [foo.9.18 1])
                                       0)])
...
(check-by-interp '(module (if (not (>= 1622421353 94202816))
                              (let ([foobar.7.2 -2027378735]
                                    [bar.5.1 9223372036854775807])
                                9223372036854775807)
                              (let ([foobar.4.4 -9223372036854775808]
                                    [bat.8.3 1])
                                9223372036854775807))))
(check-by-interp '(module (let ([bar.4.2 (* -1915251883 1)]
                                [foo.7.1 (let ([ball.9.5 -9223372036854775808]
                                               [bar.0.4 356482613]
                                               [foo.7.3 9223372036854775807])
                                           foo.7.3)])
                            (let ([ball.5.8 foo.7.1]
                                  [ball.9.7 bar.4.2]
                                  [bat.3.6 -1601764542])
                              -9223372036854775808))))
(check-by-interp '(module (define L.func.0.1
                          ...
                          (if (!= -9223372036854775808 -719419034)
                              (let ([foo.1.9 9223372036854775807]
                                    [bar.4.8 0]
                                    [foobar.5.7 9223372036854775807])
                                foobar.5.7)
...
(check-by-interp '(module (define L.fn.0.1 (lambda (ball.9.1) (call L.x.1.2 ball.9.1 ball.9.1)))
                          (define L.x.1.2
                            (lambda (ball.6.3 ball.9.2)
                              (if (not (false))
                                  (let ([foobar.4.6 ball.9.2]
                                        [foo.1.5 (let ([foo.1.8 9223372036854775807]
                                                       [bar.5.7 ball.6.3])
                                                   9223372036854775807)]
                                        [ball.9.4 -2115998024])
                                    -1101891150)
                                  (let ([bat.7.11 (* -1594085017 ball.9.2)]
                                        [ball.9.10 (let ([bat.7.13 -999943825]
                                                         [foobar.0.12 ball.6.3])
                                                     593362223)]
                                        [foobar.0.9 0])
                    ...
                    (if (if (= 1 -1319463227)
                            ...
                            (!= -1520052689 1))
                        (let ([foo.8.16 -9223372036854775808]
                              [foobar.0.15 9223372036854775807]
                              [ball.2.14 744570222])
                          foo.8.16)
                        (let ([bar.5.19 -133225634]
                              [foo.8.18 -9223372036854775808]
                              [ball.6.17 -9223372036854775808])
                          bar.5.19))))
(check-by-interp '(module (let ([bat.2.3 9223372036854775807]
                                [foobar.3.2 (let ([ball.6.6 1]
                                                  [bar.4.5 -9223372036854775808]
                                                  [ball.1.4 9223372036854775807])
                                              1)]
                                [bar.4.1 (let ([foobar.7.8 -1722042928]
                                               [foobar.9.7 0])
                                           1)])
                            ...
                            (lambda (ball.7.6 bar.8.5 bat.1.4 ball.2.3)
                              (if (true)
                                  (if (if (> 182929845 bar.8.5)
                                      ...
                                      bat.1.4)
                                  (let ([bar.5.8 (+ -9223372036854775808 -9223372036854775808)]
                                        [ball.0.7 (let ([bat.1.11 -1747554427]
                                                        [ball.2.10 ball.7.6]
                                                        [ball.0.9 9223372036854775807])
                                                    bat.1.11)])
                                    ball.0.7))))
                    (define L.x.2.3
                      (lambda ()
                        (if (< 0 9223372036854775807)
                            1083014636
                            (let ([bat.4.14 (if (<= 1 9223372036854775807) 9223372036854775807 1)]
                                  [bar.5.13 (* 0 0)]
                                  [bat.1.12 (* 1942355770 9223372036854775807)])
...
(check-by-interp
 '(module (define L.proc.0.1
            ...
            (lambda (bat.2.9 foo.4.8 foobar.0.7 foobar.1.6 bat.3.5 ball.9.4 bar.6.3)
              (if (if (true)
                      ...
                      (if (>= ball.9.4 bar.6.3)
                          (> bat.3.5 bat.3.5)
                          (< bat.2.9 bat.2.9)))
                  (let ([foo.4.11 (let ([bat.2.14 bar.6.3]
                                        [ball.9.13 -144518674]
                                        [bar.6.12 foobar.1.6])
                                    bat.3.5)]
                        [bat.3.10 (if (< bat.2.9 9223372036854775807) 0 -9223372036854775808)])
    ...
    (let ([foobar.1.19 (+ 1 -1806804688)]
          [ball.7.18 (+ 1880531761 1)])
      (if (<= 0 foobar.1.19) 0 9223372036854775807))))
(check-by-interp '(module (if (!= -107521481 -1873066368)
                              (* 1275956981 1)
                              (let ([bat.4.2 1318401057]
                                    [foobar.7.1 -542033033])
                                -288327503))))
(check-by-interp '(module (define L.fn.0.1
                            (lambda ()
                              (let ([bat.3.2 (let ([foobar.4.5 9223372036854775807]
                                                   [bat.0.4 684249837]
                                                   [bar.7.3 1])
                                               (if (= 2084897481 bar.7.3) foobar.4.5 bar.7.3))]
                                    [ball.9.1 (if (let ([ball.1.7 0]
                                                        [bat.5.6 1774106288])
                                                    (> 0 bat.5.6))
                                                  (let ([bat.0.9 9223372036854775807]
                                                        [foo.6.8 811516044])
                                                    bat.0.9)
                                                  (if (< -1327371506 9223372036854775807)
                                                      ...
                                                      9223372036854775807))])
                                (call L.fn.0.1))))
                          (let ([bat.5.11 (if (= 1517267005 605514120) -1228391641 -1926002282)]
                                [ball.9.10 (if (> 0 1) 9223372036854775807 0)])
                            (* 25767341 0))
                    ))
(check-by-interp '(module (define L.func.0.1
                            (lambda ()
                              (if (false)
                                  (if (not (!= -1086072832 9223372036854775807))
                                      (let ([bar.4.3 793954259]
                                            [bat.0.2 0]
                                            [bat.6.1 -9223372036854775808])
                                        9223372036854775807)
                                      (+ 620579510 1))
                                  (if (true)
                                      9223372036854775807
                                      (let ([ball.8.5 716210269]
                                            [bat.0.4 -524345693])
                                        -9223372036854775808)))))
                          (define L.fn.1.2
                            (lambda (bat.0.7 bat.6.6)
                              (if (if (true)
                                  ...
                                  (let ([ball.5.10 (let ([foo.3.12 bat.0.7]
                                                         [ball.5.11 bat.6.6])
                                                     foo.3.12)]
                                        [foo.3.9 (+ 1995506902 -677333479)]
                                        [bar.4.8 (let ([bar.9.15 bat.0.7]
                                                       [bat.0.14 -1273465885]
                                                       [bat.6.13 148631858])
                                                   bat.0.14)])
                                    bat.6.6))))
                    (let ([bar.7.18 -1536502942]
                          [bat.0.17 -1557455680]
                          [bat.6.16 (let ([bat.6.21 -1112875927]
                                          [ball.5.20 9223372036854775807]
                                          [foo.3.19 9223372036854775807])
                                      ball.5.20)])
                      (if (<= bar.7.18 bat.0.17) 1132619346 0))))
(check-by-interp '(module (if (true)
                              (let ([foobar.3.2 -9223372036854775808]
                                    [foo.8.1 9223372036854775807])
                                foobar.3.2)
                              (let ([foo.5.5 -9223372036854775808]
                                    [foo.8.4 1929590320]
                                    [foobar.2.3 0])
                                0))))
(check-by-interp '(module (let ([bar.1.3 (if (> 9223372036854775807 1398326902)
                                             9223372036854775807
                                             9223372036854775807)]
                                [bat.3.2 0]
                                [foo.0.1 (let ([bat.3.5 -9223372036854775808]
                                               [ball.9.4 -1747244286])
                                           ball.9.4)])
                            (let ([foo.0.7 -1923830755]
                                  [bat.5.6 bar.1.3])
                              bat.5.6))))
(check-by-interp '(module (define L.proc.0.1
                            (lambda ()
                              (let ([bat.6.3 (if (true)
                                                 0
                                                 (* 1 1))]
                                    [foo.9.2 -9223372036854775808]
                                    [bar.0.1 (* 672116958 1)])
                                (if (< bar.0.1 980303276)
                                    (+ 9223372036854775807 bar.0.1)
                                    (let ([bar.2.5 -667597146]
                                          [foo.9.4 1438122241])
                                      1900048603)))))
                          (define L.proc.1.2
                            (lambda ()
                              (if (true)
                                  (if (if (< 0 -9223372036854775808)
                                      ...
                                      -935154891
                                      (let ([ball.3.7 -1449852093]
                                            [bat.6.6 -9223372036854775808])
                                        1))

...
(define (check-input p)
  (if (values-lang-v5? p)
      p
      (error (~a (pretty-format p) "\n is not a semantically valid " "values-lang-v5" " program"))))
...
(define-syntax-rule (check-by-interp p)
  (check-equal? (interp-values-lang-v5 (check-input p))
                (interp-values-unique-lang-v5 (check-output (uniquify p)))))
...
(check-by-interp '(module (define func.0
                            (lambda (bar.6)
                              (let ([bat.2 (let ([foo.0 (+ -9223372036854775808 -9223372036854775808)]
                    ...
                    (call x.1)))
(check-by-interp '(module (define x.0 (lambda (ball.8 ball.6 foobar.1) (call x.0 foobar.1 ball.8 0)))

...
(define (check-output p)
  (if (imp-cmf-lang-v5? p)
      p
      (error (~a (pretty-format p) "\n is not a semantically valid " "imp-cmf-lang-v5" " program"))))

(define-syntax-rule (check-by-interp p)
  (check-equal? (interp-proc-imp-cmf-lang-v5 (check-input p))
                (interp-imp-cmf-lang-v5 (check-output (impose-calling-conventions p)))))
                          ...
                          (define L.fn.1.2
                            (lambda (bat.2.9 foo.4.8 foobar.0.7 foobar.1.6 bat.3.5 ball.9.4 bar.6.3)
                              (if (if (true)
                                  ...
                                  -274219017)))
                    (define L.proc.2.3
                      (lambda (ball.5.17 bat.2.16 ball.9.15) (+ bat.2.16 9223372036854775807)))
                    (begin
